    ...
    if (req.body.BlogTitle && req.body.BlogContent) {
      ...
      data
        ...
        .then((result) => {
          res
            .status(200)
            .send({ msg: "Blog Published Successfully !", redirect: false });
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      return res.send({
        msg: "Blog Title or Content Can't Be Empty",
        redirect: false,
      });
    }
  } else {
...
router.put("/updateBlog/:id", userAuth, async (req, res) => {
  try {
    ...
    if (BlogImage) {
      UpdatedBlog.BlogImage = BlogImage;
    }
    ...
    await blog
      .findByIdAndUpdate(req.params.id, { $set: UpdatedBlog })
      .then(() => {
